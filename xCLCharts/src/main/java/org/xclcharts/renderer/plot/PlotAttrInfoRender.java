...
public class PlotAttrInfoRender extends PlotAttrInfo{
	...
	{		
		...
		for(int i=0;i<mAttrInfo.size();i++)
		{
			...
			String x100 = "x100";
			if (info.contains(x100)) {
				info = info.replace(x100, "");
				canvas.drawText(info, mPosPintF.x - 6, mPosPintF.y, mAttrInfoPaint.get(i));

				Paint paintT = mAttrInfoPaint.get(i);
				paintT.setTextSize(12);
				canvas.drawText(x100, mPosPintF.x + radius-2, mPosPintF.y, paintT);
			} else {
